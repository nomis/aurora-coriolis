...
#include "htdocs/list_presets.xml.gz.h"
...
WebInterface::WebInterface(App &app) : app_(app) {
	...
	server_.add_static_content("/" + app_.immutable_id() + "/list_presets.xml",
		"application/xslt+xml", gzip_immutable_headers, htdocs_list_presets_xml_gz);
}
...
bool WebInterface::set_preset(WebServer::Request &req) {
	...
	if (preset_name.empty()) {
		...
		if (config.default_bus().empty()) {
			...
			if (!message) {
				logger_.info("Preset set to %.*s%s by %s",
					static_cast<int>(preset_name.size()), preset_name.begin(),
					set_default ? " (default)" : "", req.client_address().c_str());
			} else {
				logger_.trace("Failed to set preset to %.*s%s by %s (%s)",
					static_cast<int>(preset_name.size()), preset_name.begin(),
					set_default ? " (default)" : "", req.client_address().c_str(),
					message);
			}
